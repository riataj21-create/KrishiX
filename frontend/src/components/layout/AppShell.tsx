...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#080b14] text-[#f5f1e8]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-white/10 bg-[#0d1020] px-4 py-6 lg:flex">
        <div className="flex items-center gap-3 px-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#5b4bdb] font-semibold text-white">K</div>
          <div><p className="text-[11px] uppercase tracking-[0.2em] text-[#a9a8b3]">KrishiX</p><p className="text-sm font-semibold">Market intelligence</p></div>
        </div>
        ...
        <div className="mt-auto rounded-2xl border border-white/10 bg-[#15182a] p-3">
...
function NavItem({ to, label, icon }: { to: string; label: string; icon: React.ReactNode }) {
  return <NavLink to={to} className={({ isActive }) => `group flex items-center justify-between rounded-xl px-3 py-2.5 text-sm transition ${isActive ? 'bg-[#24152f] text-white' : 'text-[#a9a8b3] hover:bg-white/5 hover:text-white'}`}><span className="flex items-center gap-3">{icon}{label}</span><ChevronRight className="h-3.5 w-3.5 opacity-0 transition group-hover:opacity-60" /></NavLink>;
}
