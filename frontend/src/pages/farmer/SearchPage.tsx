...
export default function SearchPage() {
  ...
  return (
    <div className="min-h-screen bg-[#080b14] p-6 text-[#f5f1e8]">
      <div className="mx-auto max-w-6xl">
        <div className="glass-panel mb-6 flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between">
          <div><p className="eyebrow">Discover / markets</p><h1 className="editorial-title mt-2">Explore where your crop moves</h1></div>
          <div className="flex w-full max-w-md items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-3 py-2 backdrop-blur-md">
        ...
        <div className="mb-5 flex items-center gap-3">
          ...
          <input id="state" className="input max-w-xs" value={state} onChange={(event) => setState(event.target.value)} />
        </div>

        {loading ? <div className="glass-panel flex items-center justify-center p-16 text-[#c8c4d7]"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading latest available prices...</div> : markets.length === 0 ? <div className="glass-panel p-12 text-center text-[#a9a8b3]">No price records found for this commodity and state.</div> : (
