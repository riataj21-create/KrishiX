...
export default function OpportunitiesPage() {
  ...
  return <div className="page-container min-h-screen"><div className="mb-8 border-b border-white/10 pb-6"><p className="text-xs uppercase tracking-[0.18em] text-[#8b5cf6]">Decision engine</p><h1 className="mt-2 text-4xl font-semibold">Opportunities</h1><p className="mt-2 text-sm text-[#a9a8b3]">See where your current lot can actually be sold.</p></div>{loading ? <div className="flex items-center gap-2 border border-white/10 bg-[#15182a] p-8 text-sm text-[#a9a8b3]"><Loader2 className="h-4 w-4 animate-spin" /> Loading opportunities...</div> : <><div className="mb-6 border border-white/10 bg-[#15182a] p-5"><p className="text-xs uppercase tracking-[0.15em] text-[#a9a8b3]">Current lot</p><p className="mt-2 text-xl font-semibold">{lot?.commodity_name || 'Lot'} · {lot?.quantity_kg || (lot?.quantity || 0) * 100} kg · {lot?.quality_grade || 'Quality not specified'}</p><p className="mt-1 text-sm text-[#a9a8b3]">{lot?.district}, {lot?.state} · Sell by {lot?.sell_by}</p></div>{recommendation && <div className="mb-6 border border-[#d6a84f]/30 bg-[#24152f] p-4 text-sm text-[#f5f1e8]">{recommendation}</div>}{items.length === 0 ? <div className="border border-dashed border-white/15 bg-[#15182a] p-10 text-center text-sm text-[#a9a8b3]">No executable opportunities were found for this lot.</div> : <div className="space-y-4">{items.map((item, index) => <OpportunityCard key={item.id} item={item} primary={index === 0} />)}</div>}</>}</div>;
}
